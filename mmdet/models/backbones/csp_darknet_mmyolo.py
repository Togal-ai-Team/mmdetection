...
class YOLOv5CSPDarknet(BaseBackbone):
    ...
    def build_stem_layer(self) -> nn.Module:
        ...
        return ConvModule(
            self.input_channels,
            make_divisible_mmyolo(self.arch_setting[0][0], self.widen_factor),
            kernel_size=6,
            stride=2,
            padding=2,
    ...
    def build_stage_layer(self, stage_idx: int, setting: list) -> list:
        ...
        in_channels = make_divisible_mmyolo(in_channels, self.widen_factor)
        out_channels = make_divisible_mmyolo(out_channels, self.widen_factor)
        num_blocks = make_round_mmyolo(num_blocks, self.deepen_factor)
        stage = []
...
@MODELS.register_module()
class MMYOLOXCSPDarknet(BaseBackbone):
    ...
    def build_stem_layer(self) -> nn.Module:
        ...
        return Focus(
            3,
            make_divisible_mmyolo(64, self.widen_factor),
            kernel_size=3,
            norm_cfg=self.norm_cfg,
            act_cfg=self.act_cfg)
